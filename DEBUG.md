...
- **`npm: command not found`:** From the repo root run **`./scripts/install-desktop.sh`** (or `./install.sh`) — it installs portable Node if needed, runs **`npm install`** (including **NW.js** via the `nw` package), and verifies **`node_modules/.bin/nw`**. Or install Node from [nodejs.org](https://nodejs.org/) and run `npm install` yourself.
