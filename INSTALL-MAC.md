...
- **Conda `(base)` / mixed Node**: A conda environment can put a different `node`/`npm` on your `PATH` than the one used when `node_modules` was built. Prefer **`conda deactivate`** before `./scripts/install-desktop.sh`. To simulate a clean Mac install in a disposable folder, see **[TESTING.md](TESTING.md)**.
